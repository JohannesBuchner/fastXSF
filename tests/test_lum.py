...
from fastxsf.flux import energy_flux, photon_flux, luminosity, frac_overlap_interval
import os
...
data = fastxsf.load_pha(os.path.join(os.path.dirname(__file__), '../example/179.pi'), 0.5, 8)
...
def test_powerlaw_fluxes():
    pl = fastxsf.x.zpowerlw(energies=energies, pars=[1, 0])
    ...
    zpl0 = fastxsf.x.zpowerlw(energies=energies, pars=[1, 0.01])
    ...
    zpl = fastxsf.x.zpowerlw(energies=energies, pars=[1, 0.5])
    ...
    zpl3 = fastxsf.x.zpowerlw(energies=energies, pars=[3, 0.5])
